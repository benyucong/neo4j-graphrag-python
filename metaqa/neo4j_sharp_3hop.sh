...
# For 3-hop, we need to distribute budget across 3 levels: d1, d2, d3
# Work budget W(L) ~= K * L ; then d1 ~ W^(1/3), d2 ~ W^(1/3), d3 ~ W^(1/3)
# This gives cubic budget scaling: W = d1 * d2 * d3
...
# Limits to sweep (same as before)
# ALL_LIMITS=()
# for L in $(seq 1 15); do ALL_LIMITS+=("$L"); done
# ALL_LIMITS+=(16 20 25 30 50 80 100 150 200 250 300)
ALL_LIMITS=(150 200 250 300)  # temp for quick testing
...
"${VLLM_CMD[@]}" > logs_sharp_3hop/vllm.out 2> logs_sharp_3hop/vllm.err &
VLLM_PID=$!
...
while true; do
  if ! kill -0 "$VLLM_PID" 2>/dev/null; then
    ...
    tail -n 100 logs_sharp_3hop/vllm.err >&2 || true
    tail -n 50 logs_sharp_3hop/vllm.out >&2 || true
    exit 1
  fi
...
W = max(1, K*L)
# For 3-hop: distribute budget as cube root for each level
# d1 = d2 = d3 = W^(1/3)
d_base = max(1, int(round(W ** (1.0/3.0))))
...
# ----------------- Run batch -----------------
echo "[run] Linear-budget 3-hop sweep with K=${K}; fixed TOPK=${TOPK_FIXED}; limits: 1-15 plus 16 20 25 30 50 100" >&2

for L in "${ALL_LIMITS[@]}"; do
  # derive d1,d2,d3 from linear budget (cube root distribution)
  ...
  OUTPUT_PATH="${BASE_OUTPUT_DIR}/answers_merged_sharp_3hop_limit${L}.jsonl"
  ...
  python examples/retrieve/batch_answers_from_dataset_3hop.py \
    ...
    || { echo "${error_prefix} Batch answering failed for limit=$L" >&2; exit 1; }
...
echo "[done] Linear-budget 3-hop sweep complete" >&2
