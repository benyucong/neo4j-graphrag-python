...
"${VLLM_CMD[@]}" > logs_sharp/vllm.out 2> logs_sharp/vllm.err &
VLLM_PID=$!
...
while true; do
  ...
  if ! kill -0 "$VLLM_PID" 2>/dev/null; then
    ...
    echo "--- logs_sharp/vllm.err (tail -n 100) ---" >&2
    tail -n 100 logs_sharp/vllm.err >&2 || true
    echo "--- logs_sharp/vllm.out (tail -n 50) ---" >&2
    tail -n 50 logs_sharp/vllm.out >&2 || true
    exit 1
  fi
...
OUTPUT_PATH=${OUTPUT_PATH:-outputs/answers_merged_sharp.jsonl}
mkdir -p "$(dirname "$OUTPUT_PATH")"
python examples/retrieve/batch_answers_from_dataset.py \
  --input "$DATASET_PATH" \
  --output "$OUTPUT_PATH" \
  --aggregate min --d1 60 --d2 60 --limit 10 --normalize none --topk 10 \
