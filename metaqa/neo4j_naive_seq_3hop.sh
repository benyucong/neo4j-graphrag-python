...
"${VLLM_CMD[@]}" > logs_naive_seq_3hop/vllm.out 2> logs_naive_seq_3hop/vllm.err &
VLLM_PID=$!
...
while true; do
  ...
  echo "--- logs_naive_seq_3hop/vllm.err (tail -n 100) ---" >&2
  tail -n 100 logs_naive_seq_3hop/vllm.err >&2 || true
  echo "--- logs_naive_seq_3hop/vllm.out (tail -n 50) ---" >&2
  tail -n 50 logs_naive_seq_3hop/vllm.out >&2 || true
    exit 1
  fi
...
echo "[run] Generating answers over dataset via sequential 3-path 3-hop queries" >&2
DATASET_PATH=${DATASET_PATH:-datasets/vanilla_paths_joined_3hop.jsonl}
# DATASET_PATH=${DATASET_PATH:-datasets/vanilla_demo.jsonl}
OUTPUT_PATH=${OUTPUT_PATH:-outputs/answers_merged_naive_seq_3hop.jsonl}
mkdir -p "$(dirname "$OUTPUT_PATH")"
python examples/retrieve/batch_answers_from_dataset_seq_3hop.py \
  --input "$DATASET_PATH" \
  --output "$OUTPUT_PATH" \
  --aggregate min --d1 150 --d2 150 --d3 150 --limit 0 --normalize none --topk 1000000 \
