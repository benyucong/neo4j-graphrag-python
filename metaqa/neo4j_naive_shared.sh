...
"${VLLM_CMD[@]}" > logs_naive_shared/vllm.out 2> logs_naive_shared/vllm.err &
VLLM_PID=$!
...
while true; do
  ...
  echo "--- logs_naive_shared/vllm.err (tail -n 100) ---" >&2
  tail -n 100 logs_naive_shared/vllm.err >&2 || true
  echo "--- logs_naive_shared/vllm.out (tail -n 50) ---" >&2
  tail -n 50 logs_naive_shared/vllm.out >&2 || true
    exit 1
  fi
...
echo "[run] Generating answers over dataset via merged 3-path queries (shared distance computation)" >&2
DATASET_PATH=${DATASET_PATH:-datasets/vanilla_paths_joined.jsonl}
OUTPUT_PATH=${OUTPUT_PATH:-outputs/answers_merged_naive_shared.jsonl}
