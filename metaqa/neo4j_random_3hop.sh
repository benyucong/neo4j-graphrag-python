...
BASE_OUTPUT_DIR=${BASE_OUTPUT_DIR:-random_outputs_3hop}
...
"${VLLM_CMD[@]}" > logs_random_3hop/vllm.out 2> logs_random_3hop/vllm.err &
VLLM_PID=$!
...
while true; do
  if ! kill -0 "$VLLM_PID" 2>/dev/null; then
    ...
    tail -n 100 logs_random_3hop/vllm.err >&2 || true
    tail -n 50 logs_random_3hop/vllm.out >&2 || true
    exit 1
  fi
...
for L in "${ALL_LIMITS[@]}"; do
  ...
  OUTPUT_PATH="${BASE_OUTPUT_DIR}/answers_random_3hop_limit${L}.jsonl"
  echo "${run_prefix} limit=${L}, d1=${D1}, d2=${D2}, d3=${D3}, W~${W} -> ${OUTPUT_PATH}" >&2

  python examples/retrieve/batch_answers_from_dataset_random_3hop.py \
    --input "$DATASET_PATH" \
    --output "$OUTPUT_PATH" \
    --aggregate min \
    --normalize none \
