...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Aggregate per-limit 3-hop prediction files (answers_merged_sharp_3hop_limitX.jsonl) to CSV with avg Neo4j time and accuracy metrics."
    )
    p.add_argument(
        "--glob", default="outputs/answers_merged_sharp_3hop_limit*.jsonl", help="Glob pattern for prediction JSONL files"
    )
    p.add_argument(
        "--hf-dataset", default="Benyucong/vanilla-hop3", help="HF dataset with gold a_entity field"
    )
    ...
    p.add_argument(
        "--output-csv", default="outputs/limit_sweep_metrics_3hop.csv", help="Destination CSV file"
    )
    p.add_argument(
...
def load_throughput(jsonl_path: Path) -> float:
    """Load throughput_rows_per_sec from corresponding .metrics.json file."""
    metrics_path = Path(str(jsonl_path) + ".metrics.json")
    if not metrics_path.exists():
        return 0.0
    try:
        data = json.loads(metrics_path.read_text(encoding="utf-8"))
        return float(data.get("throughput_rows_per_sec", 0.0))
    except Exception:
        return 0.0
...
def main() -> None:
    ...
    with out_path.open("w", encoding="utf-8", newline="") as f_csv:
        ...
        for limit, path in selected:
            ...
            throughput = load_throughput(path)
            row = {
                "limit": limit,
                "samples": samples,
                ...
                "neo4j_p90_time_ms": round(p95_neo4j, 2),
