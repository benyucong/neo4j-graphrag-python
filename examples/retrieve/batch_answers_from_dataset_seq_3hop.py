...
def parse_args():
    p = argparse.ArgumentParser(
        description="Batch: read dataset JSONL, run 3-hop queries per path sequentially, and generate LLM answers."
    )
    p.add_argument("--input", required=True, help="JSONL dataset path")
    p.add_argument("--output", default="outputs/answers_merged_seq_3hop.jsonl")
...
def run_query_3hop(
    ...
    """Run a single 3-hop query for one path."""
    if normalize == "none":
    ...
    qry = (
        ...
        "d1 AS d1, d2 AS d2, d3 AS d3, (d1 + d2 + d3) AS score\n"
        "ORDER BY score ASC\n"
    )
