...
def parse_args():
    p = argparse.ArgumentParser(
        description="Batch: read dataset JSONL, run 3-hop queries per path sequentially with random shedding, and generate LLM answers."
    )
    p.add_argument("--input", required=True, help="JSONL dataset path")
    p.add_argument("--output", default="outputs/answers_merged_random_seq_3hop.jsonl")
...
def run_query_3hop_random(
    ...
    if normalize == "none":
        ...
        w3_expr = "apoc.text.replace(apoc.text.replace(apoc.text.replace(apoc.text.replace(toLower($wanted3),'freebase.',''),'_',''),'.',''),'/','')"
    ...
    # Build query with random ordering before limit
    qry = (
        f"WITH {w1_expr} AS nw1, {w2_expr} AS nw2, {w3_expr} AS nw3\n"
        "MATCH (s:Resource {id: $startId})-[r1:REL]-(m1:Resource)-[r2:REL]-(m2:Resource)-[r3:REL]-(o:Resource)\n"
        f"WITH s, r1, m1, r2, m2, r3, o, {p1_expr} AS np1, {p2_expr} AS np2, {p3_expr} AS np3, nw1, nw2, nw3\n"
        "WITH s, r1, m1, r2, m2, r3, o, "
        "apoc.text.distance(np1, nw1) AS d1, "
        "apoc.text.distance(np2, nw2) AS d2, "
        "apoc.text.distance(np3, nw3) AS d3\n"
        "WHERE d1 <= $d1 AND d2 <= $d2 AND d3 <= $d3\n"
        "WITH s, r1, m1, r2, m2, r3, o, d1, d2, d3, (d1 + d2 + d3) AS score\n"
        "ORDER BY rand()\n"  # Random ordering instead of score-based
    )
    if limit and limit > 0:
        qry += "LIMIT $limit\n"
    qry += "RETURN s.id AS s, r1.pred AS p1, m1.id AS m1, r2.pred AS p2, m2.id AS m2, r3.pred AS p3, o.id AS o, d1, d2, d3, score"
...
def build_llama_inst_prompt(question: str, evidence: List[dict]) -> str:
    sys_txt = (
        "Based on the reasoning paths (randomly sampled), answer the question. "
...
def aggregate_rows_3hop(rows_by_path: Dict[str, List[dict]], aggregate: str) -> List[dict]:
    """Merge by (s, p1, m1, p2, m2, p3, o); keep per-path scores, then aggregate to final score."""
    merged: Dict[Tuple[str, str, str, str, str, str, str], Dict[str, Any]] = {}
    for label, rows in rows_by_path.items():
        for r in rows:
            key = (r.get("s"), r.get("p1"), r.get("m1"), r.get("p2"), r.get("m2"), r.get("p3"), r.get("o"))
            entry = merged.setdefault(
                key,
                {
                    "s": key[0], "p1": key[1], "m1": key[2], "p2": key[3], "m2": key[4], "p3": key[5], "o": key[6],
                    "scoreA": None, "scoreB": None, "scoreC": None
                },
            )
            if label == "A":
    ...
    try:
        with inp.open("r", encoding="utf-8") as f_in:
            for line in f_in:
                ...
                for idx in (1, 2, 3):
                    sp = split_path_3hop(row.get(f"path{idx}") or "")
                    if sp:
                        triples.append(sp)
                ...
                if not triples:
                    items.append({
                        ...
                        "triples": triples, "neo4j_time_us": 0, "skip_llm": True
                    })
                    done += 1
                    continue
                ...
                labels = ["A", "B", "C"]
                for i, (p1, p2, p3) in enumerate(triples[:3]):
                    try:
                        recs = list(
                            run_query_3hop_random(
                                driver,
                                args.database,
                                str(q_entity),
                                p1,
                ...
                evidence_all = aggregate_rows_3hop(rows_by_path, args.aggregate)
                if not evidence_all:
                    items.append({
                        "id": rid, "question": question, "q_entity": q_entity,
                        "triples": triples, "neo4j_time_us": neo4j_time_us, "skip_llm": True
                    })
                    done += 1
                    continue
                ...
                items.append({
                    "id": rid, "question": question, "q_entity": q_entity,
                    "triples": triples, "neo4j_time_us": neo4j_time_us,
        ...
        for i, item in enumerate(items):
            if item.get("skip_llm"):
                out = {
                    ...
                    "paths": item["triples"], "answer": [], "no_evidence": True,
