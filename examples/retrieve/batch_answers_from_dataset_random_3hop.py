...
def parse_args():
    p = argparse.ArgumentParser(
        description="Batch: read dataset JSONL, run merged 3-hop queries with random shedding, and generate LLM answers."
    )
    p.add_argument("--input", required=True, help="JSONL dataset path")
    p.add_argument("--output", default="outputs/answers_merged_random_3hop.jsonl")
...
def run_query_merged_3hop_random(
    driver,
    db: str,
    start_id: str,
    ...
    """
    Run a merged 3-hop query across up to 3 paths with random shedding.
    Each path is (pred1, pred2, pred3).
    Returns rows: s -[p1]-> m1 -[p2]-> m2 -[p3]-> o
    with scoreA, scoreB, scoreC, combined score.
    Uses ORDER BY rand() instead of ORDER BY score for random selection.
    """
    # Pad to 3 paths
    ...
    # NOTE: First/second/third hop use lowest distance; only final selection is randomized
    qry = f"""
    ...
    // ---------- First hop: s -[r1]- m1 (global prune by lowest distance to $d1) ----------
    CALL {{
      WITH s, w1a, w1b, w1c
    ...
    // ---------- Second hop: m1 -[r2]- m2 (per-m1 prune by lowest distance to $d2) ----------
    CALL {{
      WITH m1, w2a, w2b, w2c
    ...
    // ---------- Third hop: m2 -[r3]- o (per-m2 prune by lowest distance to $d3) ----------
    CALL {{
      WITH m2, w3a, w3b, w3c
    ...
    // Final selection is random (not score-ordered)
...
def build_llama_inst_prompt(question: str, evidence: List[dict]) -> str:
    sys_txt = (
        "Based on the reasoning paths (randomly sampled), please answer the given question. "
